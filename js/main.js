 ...
 let students = [];
 class Person {
     constructor(name,techPoints,lifePoints,status){
        this.name = undefined;
...
function checkNotes() {
    ...
    for(var i in newStudents){
       $('#student').append(`<div class="addStudents"><h1><b>STUDENT:</b></h1><br><p><b>Name:</b>${newStudents[i].name }</p><p><b>Tech Points</b>:${newStudents[i].techPoints }</p><p><b>Life Points:</b>${newStudents[i].lifePoints }</p><p><b>Status:</b>${newStudents[i].status }</p></div>`);
   }
